    ...
    <div class="container">
        ...
        <div id="status-tab" class="tab-content">
            <div class="info-group">
                ...
                <span class="info-value"><span id="system_status_badge" class="badge badge-green"><svg class="status-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill='#10b981' fill-opacity="0.2" stroke='#10b981' stroke-width="2"/><path d="M8.5 12.5L10.5 14.5L15.5 9.5" stroke='#10b981' stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>Opérationnel</span></span>
            </div>
            <div class="info-group">
        ...
        function fetchStatus() {
            fetch('/api/status')
                ...
                .then(data => {
                    ...
                    if (data.ip_addr && data.ip_addr !== data.gateway_addr) {
                        document.getElementById('ip_addr').innerHTML = data.ip_addr + getStatusSvg('green', 'Internet disponible');
                    } else {
                        document.getElementById('ip_addr').innerHTML = 'Non connecté' + getStatusSvg('red', 'Hors ligne');
                    }
                    
                    // 2. Passerelle (avec ou sans internet)
                    ...
                    // 5. firmware (pas de firmware de prod, version deja demare, derniere version en place)
                    ...
                    if (document.getElementById('update_interval') && data.update_interval) {
